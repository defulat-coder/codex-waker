...
Management surfaces preserve that shell while choosing the smallest structure that makes state scannable. Memory uses a bordered 250px index beside a flexible detail pane. Automations use auto-fitting cards with a 280px minimum. Workflows use a desktop master-detail surface: a compact definition index, a strict JSON editor with server validation, a read-only path diagram, immutable version/diff/rollback controls, and a bounded run/trace ledger. Board pairs a filterable List/Lanes work surface with a protected detail dock. Skills pairs a searchable inventory/library grid with a metadata and instruction dock. Capabilities use the same tab grammar, with two equal permission cards. Session attachments, artifacts, and file changes live in a 390px right-side panel separated from the thread by one quiet rule.

Goal 5 Workflow and Goal 6 Board/Skills design are PC-desktop only, matching the repository's current frontend scope. Existing narrow-screen behavior documented elsewhere is historical coverage from earlier goals; it is not a new acceptance target or a requirement to introduce mobile-specific layout or interactions.
...
Automation uses the same master-detail grammar: a horizontally reachable task index on narrow widths and a full configuration/detail surface on desktop. Definitions expose schedule, timezone, bounds, max-runs, misfire policy, project, model, and thinking snapshot without hiding paused or completed state. Run history is a bounded expandable ledger: trigger, scheduled time, duration, attempt lineage, Session, result, usage, failure, cancel, and retry remain inspectable. Manual runs stay available while a schedule is paused; schedule completion and API/Event external-trigger limitations are stated rather than inferred from color.

Workflow uses the same operational grammar without pretending that a read-only projection is a visual editor. The JSON definition is the editable source of truth and accepts exactly seven node kinds: action, codex, decision, wait, ask_user, call_workflow, and terminal. Validation errors remain beside the editor. The adjacent path diagram communicates node order, branches, waits, and child calls for inspection only. Version history is immutable; diff and dry-run rollback precede an explicit apply that creates a new version. Run rows expose durable state, current node, Session, usage, input/result, human-action handoff, child workflow lineage, cancel/retry, and ordered Trace while keeping list/history responses bounded.

Every Workflow Run owns one Codex Session, even if the definition contains more than one Codex node. `waiting_input`, paused waits, child workflow calls, restart recovery, cancellation, retry lineage, and the maximum child depth of eight are written as explicit state rather than inferred from animation or color. The browser cannot forge lifecycle events; execution controls request host actions and then render persisted state.

Board is a truthful projection surface, not a second scheduler. List mode uses native table semantics and bounded load-more; Lanes groups the same server-filtered Task set into queued, active, and finished states without implying drag-to-mutate. Manual Tasks expose create/edit/delete with expected-version conflict handling. Automation and Workflow Tasks are marked managed and read-only, derive status/result/error/Session/source links from their host Run, and update through the same persisted timeline. Task detail keeps metadata, children, events, Human Actions, and source navigation together. Human Action resolve/ignore dialogs preserve focus and send the action version so stale operators cannot handle an item twice.

Skills is explicitly workspace-shared, never presented as a per-Waker toggle. Installed cards use stable locator identity because duplicate names may exist across repo `.agents/skills` and project `CODEX_HOME` host `.codex/skills`; both scopes are runtime-available when valid. Detail shows authoritative frontmatter validation, source/lock metadata, version, implicit-invocation policy, dependencies, files, integrity state, and the instruction body. Repo Skills are changed only through the pinned Skills CLI; instruction-only uploads are staged first, and install/remove confirmation states name the files that will change. Host Skills may be unmanaged, but remain readable and removable only by their exact locator.

The third-party library is discovery, not trust. Search/results may come from skills.sh, but the install dialog must say that repository instructions, scripts, and tool dependencies have not been reviewed. The product must not render install counts, source metadata, or a successful download as a security endorsement. This matches the [official Codex Skills documentation](https://developers.openai.com/codex/skills): scope comes from the filesystem location, and progressive disclosure loads full `SKILL.md` instructions only after a Skill is selected.
...
- **Don't** let the browser edit or delete derived Automation/Workflow Tasks, or represent a Lanes grouping as an independent task state store.
- **Don't** imply that skills.sh is reviewed, collapse repo and project-host Skills into one unnamed scope, or claim that a Skill is bound to one Waker.
