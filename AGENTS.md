...
- All Web animation work must use the installed `motion` dependency (`import { motion } from 'motion/react'`); do not add other JS animation libraries unless explicitly requested.
- CSS transitions are limited to simple color, border and shadow feedback. Do not add CSS `animation` or `@keyframes`; use Motion for continuous, enter/exit, gesture, layout and sequenced animation.
